...
| **OpenClaw 通信层宿主** | 承载 M-04 OpenClaw 通信层实例，管理与 OpenClaw 内核的长连接；连接状态变更通过 IPC 同步给渲染进程 |
...
| **状态栏** | OpenClaw 连接状态实时指示（绿/红圆点）、Skill 数量、App 数量 |
...
graph TB
    subgraph DesktopProcess["IntentOS Desktop (Electron 主进程)"]
        ...
        MainProcess --> ClawComm["M-04 OpenClaw 通信层"]
    ...
    ClawComm <-->|"WebSocket/gRPC"| OpenClaw
...
主进程 (ipcMain.handle() / ipcMain.on())
    ...
    │ 调用内部模块
    ▼
Skill 管理器 / 生命周期管理器 / OpenClaw 通信层 / 生成器
```
...
| **SkillApp 安全尤为关键** | SkillApp 的 UI 代码由 AI 生成，必须严格限制其能力范围，防止生成的代码访问不应访问的系统资源 | 如果 SkillApp 使用 remote，生成的 UI 代码可以直接操控主进程，构成重大安全隐患 |

**结论**：鉴于 SkillApp 的代码由 OpenClaw AI 自动生成、安全审计难以覆盖所有生成代码的特殊场景，必须选择 contextBridge 作为唯一的跨进程通信通道。
...
contextBridge.exposeInMainWorld('intentOS', {
  settings: {
    ...
    // 获取 OpenClaw 连接配置
    getClawConfig: (): Promise<ClawConfig> =>
      ipcRenderer.invoke('settings:get-claw-config'),

    // 更新 OpenClaw 连接配置
    setClawConfig: (config: ClawConfig): Promise<void> =>
      ipcRenderer.invoke('settings:set-claw-config', config),
...
sequenceDiagram
    ...
    alt 验证通过
        Desktop-->>App: 握手响应<br/>{jsonrpc:"2.0", result:{<br/>openclawEndpoint, heartbeatInterval}}
...
**对 IntentOS 的特殊意义**：SkillApp 的 UI 代码由 AI 自动生成，可能存在未预见的安全问题。contextIsolation 确保即使生成的代码有缺陷，也无法突破渲染进程沙箱访问系统资源。
...
| SkillApp 渲染进程 | `true`（强制） | AI 生成的代码安全性无法保证，必须严格沙箱化 |
...
const securityMatrix = {
  ...
  'SkillApp 渲染进程': { nodeIntegration: false, purpose: 'AI 生成代码绝不允许直接访问 Node API' },
};
```
...
graph TB
    ...
    subgraph TrustBoundary3["不信任域: AI 生成代码"]
...
| 决策项 | 选择 | 理由 |
|--------|------|------|
| **UI 框架** | React 18 | (1) 生态系统最成熟，组件库丰富（Ant Design、Radix UI 等），适合快速开发管理台界面；(2) Concurrent Mode 支持大列表渲染（Skill/App 列表）不阻塞 UI；(3) OpenClaw 生成 SkillApp UI 代码时，React 的声明式组件模型更适合 AI 代码生成（结构化、可预测）；(4) 社区规模最大，长期维护有保障 |
| **类型系统** | TypeScript 5.x | (1) IPC 通信涉及大量消息类型定义，TypeScript 的类型系统可确保主进程与渲染进程的消息契约一致；(2) 对 `contextBridge` 暴露的 API 提供编译时类型检查，减少运行时错误；(3) OpenClaw 生成的 SkillApp 代码也使用 TypeScript，保持技术栈一致 |
...
| **UI 组件库** | shadcn/ui + Tailwind CSS 4 | (1) shadcn/ui 是基于 Radix UI 的上层封装，提供高质量可复制组件，满足无障碍访问要求且便于深度定制；(2) Tailwind CSS 4 原生 CSS 层叠层支持，性能更优；(3) 可作为 OpenClaw 生成 SkillApp UI 的标准组件库，保证生成代码的一致性和质量 |
...
| TD-08 | SkillApp 渲染进程安全 | sandbox + contextIsolation + nodeIntegration:false | 仅 contextIsolation | AI 生成代码需最严格沙箱限制 | 安全需求 |