...
1. [里程碑划分](#1-里程碑划分)
   - [1.2 Phase 0 开发文档编写阶段](#12-phase-0-开发文档编写阶段w0)
2. [任务分解](#2-任务分解)
   - [2.0 开发文档编写阶段（Phase 0）](#20-开发文档编写阶段phase-0)
3. [任务依赖关系图](#3-任务依赖关系图)
4. [关键路径分析](#4-关键路径分析)
   - [4.0 开发文档阶段关键路径](#40-开发文档阶段关键路径)
5. [每阶段验收标准](#5-每阶段验收标准)
...
gantt
    ...
    M-04 通信层实现文档       :d002, after d001, 2d
    M-02 Skill管理器实现文档  :d003, after d001, 1d
    M-03 生命周期管理器实现文档:d004, after d003, 1d
    M-05 生成器实现文档       :d005, after d002, 2d
    M-01 桌面容器实现文档     :d006, after d001, 2d
    M-06 运行时实现文档       :d007, after d004, 1d
...
### 1.2 Phase 0 开发文档编写阶段（W0，2026-03-16 ~ 2026-03-27）
...
| D008 | 原地变形实现文档 | 方案B（窗口位置迁移）完整实现步骤（getBounds→SkillApp 启动→setBounds→show 时序）、方案C（后台预热+淡入淡出）实现步骤（offscreen 预热→opacity 动画→前台切换→旧窗口销毁）、`performWindowTransform()` 函数完整签名与内部逻辑、过渡时长测量方案、Spike 验证 checklist | 1 | D007 | `docs/impl/in-place-transform.md` |
| D009 | 跨模块接口契约文档 | 所有模块间接口的完整定义汇总：M-01↔M-02/M-03/M-04/M-05/M-06 的 tRPC 调用契约、M-03↔M-06 的 IPC 消息契约、M-04↔OpenClaw 的 WebSocket 消息契约、M-05↔M-03 的部署接口契约、错误码统一规范表 | 1 | D005, D007 | `docs/impl/interface-contracts.md` |
...
flowchart LR
    ...
    %% 开发文档 → 基础设施（D010 是 Phase1 的门控条件，通过 T001 传递到所有后续任务）
    D010 --> T001
...
**并行策略**：
- D003（M-02 文档）、D006（M-01 文档）可与 D002 在第 2 天（D001 完成后）并行启动
- D004（M-03 文档）可与 D002 第 2 天并行
- D007（M-06 文档）、D008（原地变形文档）可在 D004/D006 完成后立即并行

> **注意**：D009（接口契约文档）除依赖关键路径上的 D005 外，还依赖 D007（M-06 运行时文档）。D007 完成于第 4 天，D005 完成于第 5 天，因此 D007→D009 这条依赖不在关键路径上（D005 是真正的瓶颈），不影响 7 天的关键路径结论。
...
|--------|----------|----------|--------|------|
| R0 | 文档编写阶段超期或质量不足（文档不完整导致编码阶段返工，或文档间不一致引发实现偏差） | D010（审查验收）+ Section 5.0 验收门控（逐文档交叉审查，遗漏项归零才可放行） | Phase 0 | 计划中 |
