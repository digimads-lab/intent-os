...
## CR-001 迭代（已完成）：支持自定义 URL + API Key 作为 AI Provider

> **状态**：✅ 已完成（2026-03-14）
> **CR 文档**：`docs/change-requests/CR-001/CR-001.md`

### 迭代概览

| 迭代 | 目标 | 状态 |
|------|------|------|
| **Iter CR-1** | M-04 核心 Provider 实现（`CustomOpenAIProvider`、`APIKeyStore` 扩展、`AIProviderManager` 扩展、M-05 Prompt 适配） | ✅ 完成 |
| **Iter CR-2** | UI 集成与 IPC 接入（设置页/向导 UI、状态栏动态化、Provider 切换会话确认） | ✅ 完成 |
| **Iter CR-3** | 集成测试与回归（新功能单元测试、IPC 集成测试、Claude API 流程回归） | ✅ 完成 |

### 主要变更

- **M-04**：新增 `CustomOpenAIProvider`（使用 `openai` npm 包），`ProviderConfig` 改为判别联合类型，`APIKeyStore` 扩展多 Provider Key 存储，新增 2 个 IPC channel
- **M-01**：设置页新增自定义 Provider 配置表单，向导 Step 1 新增 Custom 选项，状态栏 Provider 名称动态化
- **M-05**：`buildPlanSystemPrompt`/`buildGeneratePrompt` 新增 `options.providerId` 参数，custom 模式下移除 Claude 专有引导词
- **测试报告**：`.claude/test-reports/CR-001-iteration-3.md`
