...
- 侧栏选中项高亮显示
- 主内容区切换采用淡入淡出过渡动画
- 状态栏 AI Provider 状态实时更新：显示当前 Provider 名称（如「Claude API」）及连接状态——绿色圆点 = 已连接，黄色圆点 = 验证中，红色圆点 = 不可用
...
- **Provider 下拉**：MVP 仅「Claude API (Anthropic)」可选；「OpenClaw（即将推出）」灰色展示，不可点击
...
- Provider 切换（未来版本）将触发连通性重新验证
- 隐私提示常驻显示，不可关闭，确保用户知情

**覆盖需求**：M05b（API Key 管理）、M05c（AI Provider 选择）
