...
对应 IPC 通道：`settings:get`、`settings:set`、`settings:get-provider-config`、`settings:set-provider-config`、`settings:get-api-key`、`settings:save-api-key`、`settings:delete-api-key`、`settings:test-connection`（请求），`settings:connection-status-changed`（事件推送）
